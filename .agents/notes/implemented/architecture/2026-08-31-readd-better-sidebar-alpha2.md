...
Amended 2026-09-10 (facts only): the aggregate pin sits at `dsh-better-sidebar@0.19.0`, the stable npm published 2026-09-10 whose peers declare `^0.1.5-rc.1` across the board — exactly the cohort this repo had already moved to, so no prerelease tolerance is involved. It replaced the same day's short-lived `0.19.0-alpha.1` pin (peers `^0.1.5-alpha.2`). Compared with 0.18.0 its static imports add `@deepseek-ai/dsh-session`, and `dsh.client.inject` adds the optional `@deepseek-ai/dsh-client-ui-sidebar-right` face (`peerDependenciesMeta` optional); its `cordis.patch.yml` is byte-identical to 0.18.0's, so the aggregate patch needed no regeneration. The `minimumReleaseAgeExclude` pin and the `scripts/e2e-mount.sh` scratch-profile heredoc entry follow the exact pinned version. The desktop seed resolves the aggregate's published pin at packaging time and stays untouched; its ledger entry moves when a family publish re-rolls the seed. The re-pin reaches existing `dsh-web-all` installs only through the next family release: the aggregate pins better-sidebar exactly, so the plugin manager must first see a new aggregate version before pnpm re-resolves the dependency.
...
Follow-up (2026-09-10, 0.19.0 stable round): `pnpm aggregate:check`, `pnpm docs:check`, `pnpm i18n:check`, `pnpm test:scripts`, `pnpm runtime-deps:check`, `pnpm typecheck`, and `pnpm test` pass after the re-pin; `bash scripts/e2e-mount.sh` packs the aggregate with `dsh-better-sidebar@0.19.0` resolved from the registry into a scratch profile and the Playwright lane passes again (`better-sidebar` mounted, no crash markers, `archive-manager` absent).
