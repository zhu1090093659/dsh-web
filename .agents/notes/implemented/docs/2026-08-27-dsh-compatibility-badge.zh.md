...
根 README 双语对（README.md、README.en.md）在同一个居中徽章行新增一枚静态 shields.io 徽章，内容为 `DSH | 0.1.1-rc.2`（slate 色标签段、indigo 色数值段、flat-square），链接到 npm 包 `@deepseek-ai/dsh`，位置在 users 徽章与 license 徽章之间。展示值是当前 SDK cohort 经过验证的最新 DSH 版本：与 pnpm-workspace.yaml、shared/package.json 及各包 devDependencies 钉住的 cohort 一致，且交付时等于 npm 上 `@deepseek-ai/dsh` 的最新版（0.1.1-rc.2）。该版本号在 SDK cohort 升级流程内手动随升级更新——该流程本就要对整个家族对照目标版本重新验证。徽章只做展示，不是机器校验的契约：packages/dsh-web-all/package.json 的 `dsh.engines.dsh` 仍是机器可读的兼容下限。
...
- 验证：shields 返回徽章 HTTP 200，渲染标签 DSH 与数值 0.1.1-rc.2；npm registry 上 `@deepseek-ai/dsh` 最新版即 0.1.1-rc.2。
