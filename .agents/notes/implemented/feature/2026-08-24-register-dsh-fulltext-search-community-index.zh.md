...
- 条目带 `subcategory: "dev"`（`category: "tools"` 下，dev 校验器对 tools 类的合法值为 context / browser / api / model / dev），与 dsh-plugin-hub / dsh-skill-explorer / dsh-session-insights 形态一致。
- 插件依赖 dsh-better-sidebar（外部 npm 包，聚合包依赖 `dsh-better-sidebar@0.15.2`），搜索运行在其文件管理器宿主上。商店一键安装只安装条目本身、不会解析 npm 依赖，用户需自行安装 dsh-better-sidebar；条目描述已写明该前置依赖。
- 安装/升级/卸载顺序（执行前与上游确认版本兼容范围）：安装先 dsh-better-sidebar 后 dsh-fulltext-search；升级先 dsh-better-sidebar 后 dsh-fulltext-search；卸载先 dsh-fulltext-search 后 dsh-better-sidebar。
