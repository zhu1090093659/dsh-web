...
把文件拖进窗口，输入框自动填入文件的**原路径**（不用手输深路径）：宿主优先读 OS 拖拽携带的 file:// 地址，再用 Spotlight（mdfind）按文件名全盘精确匹配，唯一命中即显示原路径；极少数无法解析时才回退为暂存副本（`~/.dsh/dsh-file-drop/`）并明确标注。填入的是纯路径，你补上任务说明再发送即可。
