...
- **Explorer (rightmost column, default 260px, range 220–500px)**: `File / Changes` two tabs; clicking a row in the file tree expands/collapses a folder, clicking a file opens it in the preview panel, and the top filename search (150ms debounce, clicking a result locates it in the tree without interrupting flow); the `Changes` tab groups the containing repository and independent nested repositories, reads each real git status, and routes diff / stage / unstage / discard to the selected repository (untracked via delete, tracked via restore, bulk discard asks for confirmation).
...
- The recursive watcher ignores changes under `node_modules` / `.git`; the SCM poll runs every 30s per workspace (each probe bounded by a 15s deadline), while repository discovery and negative verdicts use TTL caches. File edits surface via the watcher immediately; `.git`-only changes (commits/checkouts from other tools) and repository-list changes appear within one poll interval or on window focus (throttled to once per 5s).
- The browser half (`src/client/`) treats the current session cwd as the project root; switching sessions switches projects.

## Known limitations

- Nested-repository discovery skips `node_modules`, does not follow symlinked directories, and scans at most 10,000 directories per workspace pass. Adding or removing a repository can take up to the 30-second discovery-cache TTL to appear.
