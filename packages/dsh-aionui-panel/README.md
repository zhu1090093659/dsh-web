...
- **Math formulas** (#421): `$...$` inline and `$$...$$` display math in the markdown preview typeset via KaTeX. The KaTeX runtime, stylesheet and fonts ship in the package and are served same-origin via `/aionui-panel/vendor/katex.js|katex.css|fonts/*` (no CDN, offline-friendly, loopback-fenced); they load lazily only when a formula is present. Dollar signs stay currency-safe (loose `$5 and $10` pairs are plain text, `\$` is a literal), math inside code fences / spans is untouched, and without KaTeX (or on a formula error) the raw TeX stays visible as the fallback.
...
- The host half (`src/index.ts` + `src/host/`) serves directory listing, file reads (text capped at 80k chars / image data URLs), writes (mtime conflict detection), filename search (skipping .git / node_modules), git status (porcelain v1 -z) / stage / unstage / discard, and an SSE change stream (fs watching + git polling) over the `/aionui-panel/*` HTTP routes. It also serves the vendored third-party artifacts (copied from the pinned npm dependencies at build time) under `/aionui-panel/vendor/*` with etag revalidation: the mermaid IIFE bundle (`lib/assets/mermaid.min.js`), and the KaTeX runtime / stylesheet / fonts (`lib/assets/katex/`). The SSE change stream is shared across tabs through a cross-tab leader relay (Web Locks + BroadcastChannel): one stream per project browser-wide, so opening the same project in multiple tabs no longer exhausts the same-origin HTTP connection pool and hangs the panel (#383).
...
- `src/client/` — browser half: framework-agnostic state core (`store.ts`), drag engine (`drag.ts` + `hooks/useResizableSplit.ts`), DOM layout controller (`layout.ts`, appending panel tracks to the shell's three-column grid), React components (explorer / scm / preview), the mermaid enhancement (`preview/mermaid.ts` + `chat/mermaid-chat.tsx`), and the KaTeX math enhancement (`preview/katex.ts`).
