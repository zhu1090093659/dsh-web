...
- **Explorer（最右栏，默认 260px，范围 220~500px）**：`文件 / 变更` 双 tab；
  ...
  restore，批量放弃有确认）。
- **拖拽文件/目录到输入框**：文件树中的文件行与目录行均可拖拽，拖到聊天
  输入框区域松手即把相对路径（如 `deploy/base/deployment.yaml`、`deploy/base`）
  插入当前会话草稿的光标处，agent 收到消息后会自行读取该文件/目录，无需
  手动输入路径；拖拽过程中输入框上方显示高亮提示条。
