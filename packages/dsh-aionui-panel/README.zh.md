...
- **数学公式**：markdown 预览中的 `$$...$$` 块级公式与 `$...$` 行内公式经 KaTeX 渲染。KaTeX 运行时、样式表与字体打包在包内，经 `/aionui-panel/vendor/katex.js` / `katex.css` / `fonts/KaTeX_*.woff2` 同源提供（不走 CDN、离线可用、loopback 围栏）；字形颜色继承 shell 主题；运行时缺失或语法错误时回退为原始 TeX 文本。
...
- host 半区（`src/index.ts` + `src/host/`）经 `/aionui-panel/*` HTTP 路由提供目录列举、文件读取（文本 80k 字符上限 / 图片 data URL）、写入（mtime 冲突检测）、文件名搜索（跳过 .git / node_modules）、git status（porcelain v1 -z）/ stage / unstage / discard，以及 SSE 变更流（fs 监听 + git 轮询）；并经 `/aionui-panel/vendor/mermaid.js` 提供构建期从固定版本 npm 依赖拷贝的 mermaid IIFE 产物（`lib/assets/mermaid.min.js`），经 `/aionui-panel/vendor/katex.js` / `katex.css` / `fonts/KaTeX_*.woff2` 提供 KaTeX 产物（`lib/assets/katex/`，运行时 + 样式表 + 字体），均带 etag 再校验。SSE 变更流经跨标签页选主中继共享（Web Locks + BroadcastChannel），同一项目全浏览器只保留一条流，多标签页打开同一项目不再挤满同源 HTTP 连接池导致面板请求挂起（#383）。
...
- `src/client/` — browser 半区：框架无关状态核心（`store.ts`）、拖拽引擎（`drag.ts` + `hooks/useResizableSplit.ts`）、DOM 布局控制器（`layout.ts`，向 shell 的三栏 grid 追加面板轨道）、React 组件（explorer / scm / preview），以及 mermaid + KaTeX 增强（`preview/mermaid.ts` + `preview/katex.ts` + `chat/mermaid-chat.tsx`）。
