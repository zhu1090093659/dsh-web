...
import { enhanceMathPlaceholders } from './katex.ts'
...
function MarkdownViewer({
  ...
  return <EnhancedMarkdown html={html} />
}
 ...
 * flips), and math placeholders (issue #421) typeset once per html via the
 * lazily loaded vendored KaTeX. The scope marker lets the chat-transcript
 * enhancer skip this subtree (each surface owns its blocks).
 */
function EnhancedMarkdown({ html }: { html: string }): JSX.Element {
  const ref = useRef<HTMLDivElement | null>(null)
  useEffect(() => {
    const el = ref.current
    if (el === null) return undefined
    void enhanceMermaidBlocks(el, { className: previewCss.mermaidBlock, theme: mermaidTheme(shellIsDark()) })
    void enhanceMathPlaceholders(el)
