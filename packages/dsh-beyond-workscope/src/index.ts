...
import { grantTool, listTool, probeTool, readTool, revokeTool, writeTool, type WorkscopeRuntime } from './tools.ts'
...
export const BEYOND_GUIDANCE = [
  ...
  '授权生效后，仅可用 workscope_read / workscope_write 在该目录内操作（插件强制边界，越界直接拒绝并提示先授权）；workscope_list 查看本会话的授权；',
  '工作完成或用户要求时立即 workscope_revoke；会话结束授权自动撤销。',
  '纪律：先 probe 感知、再 grant 申请、确认后再读写、用完即 revoke；不要用普通文件工具直接访问工作区之外的路径（会被 DSH 沙箱或本插件拒绝）。',
  '用户提到「桌面/文档/下载/某个目录/最近的文件/整理/处理一下这个文件夹」等涉及工作区之外的需求时，即指本插件，请据此协作。',
].join('')
