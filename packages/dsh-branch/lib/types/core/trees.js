...
export const EMPTY_TREE_REGISTRY = {
    trees: [],
    current: MAIN_TREE,
    masterCounter: 0,
};
export function treeByName(registry, name) {
    return registry.trees.find(tree => tree.name === name);
}
export function nextMasterName(registry) {
 ...
 */
export function branchTreeAt(registry, stateIndex, nodeIndex, label, now) {
    const existing = registry.trees.find(tree => tree.stateIndex === stateIndex);
    if (existing !== undefined)
        return { registry, tree: existing, created: false };
...
export function withCurrent(registry, name) {
    if (name !== MAIN_TREE && treeByName(registry, name) === undefined)
        return registry;
    if (registry.current === name)
        return registry;
    return { ...registry, current: name };
}
export function isMainTree(registry) {
    return registry.current === MAIN_TREE;
}
/** Highest master number already minted (0 when none). */
export function masterNumber(registry) {
    let max = 0;
...
function parseTreeRef(value) {
    ...
    if (typeof record.name !== 'string' || record.name === '')
        return null;
    if (record.kind !== 'main' && record.kind !== 'master')
        return null;
    if (!isFiniteNumber(record.nodeIndex) || !isFiniteNumber(record.stateIndex))
        return null;
    return {
        name: record.name,
...
export function parseTreeRegistry(value) {
    if (typeof value !== 'object' || value === null)
    ...
    if (Array.isArray(record.trees)) {
        for (const entry of record.trees) {
            ...
            if (tree !== null && tree.name !== MAIN_TREE)
                trees.push(tree);
        }
    }
    const deduped = [...new Map(trees.map(tree => [tree.name, tree])).values()];
    const counter = Math.max(isFiniteNumber(record.masterCounter) ? Math.max(0, Math.floor(record.masterCounter)) : 0, masterNumber({ trees: deduped, current: MAIN_TREE, masterCounter: 0 }));
