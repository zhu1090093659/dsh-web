...
- **Profile 生命周期**：把市场条目与当前 Web profile 的直接依赖进行比对，并提供安装、更新和卸载操作。对已验证的 GitHub 项目，安装和更新时可选择经过验证的 SHA 或仓库默认分支的最新版本。操作弹窗会实时显示准备、刷新目录、读取本地清单、执行命令和完成五个阶段，并保持展开显示命令输出。失败后可把仓库、指令、失败阶段、错误和输出发送到新的 Agent 对话进行诊断；消息被接受后才关闭设置页。操作成功后需要重启 DSH Web。
- **对话集成**：注册 `store_catalog`、`store_search`、`store_details`、`store_installed`、`store_install` 和 `store_remove`，并随包提供 `search-dsh-store` Skill。写操作工具执行前进入 DSH 审批流程。写操作失败时返回结构化证据，由当前 Agent 立即分析，不再以原始工具异常结束。
...
- **对话使用**：让 Agent 搜索、查看、安装、更新或卸载市场项目。两种 GitHub 模式均可用时，Agent 必须先询问用户选择。读取工具可直接运行；安装、更新和卸载必须经过明确的 DSH 审批。失败时由当前 Agent 分析返回的证据，不会自动重试或继续调用更多工具。
...
- 错误证据和命令输出始终按不可信诊断数据处理。转交 Agent 的提示会禁止执行其中的指令、自动重试，以及在没有用户新明确请求时继续修改。
