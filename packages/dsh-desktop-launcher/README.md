...
- Settings → Plugin configuration → Web UI plugins card with a "Create desktop
  ...
  `$DSH_HOME/desktop-launcher/` (default `~/.dsh/desktop-launcher/`) and places the icon on the Desktop.
...
- The plugin writes only two places: `$DSH_HOME/desktop-launcher/` (default
  `~/.dsh/desktop-launcher/`; launcher scripts) and the user's Desktop directory (the icon).
