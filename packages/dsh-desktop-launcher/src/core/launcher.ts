...
function renderPowerShell(spec: LauncherSpec): string {
  return [
    ...
    '$dshProcess = if ($command.CommandType -eq \'ExternalScript\' -or $command.Source -match \'\\.ps1$\') {',
    ...
    '} else {',
    '  Start-Process -FilePath $command.Source -ArgumentList $arguments -WindowStyle Hidden -PassThru',
    '}',
    '',
    ...
    "    Show-PopupError '启动失败' (\"DSH 进程已异常退出（退出码：\" + $dshProcess.ExitCode + \")\")",
    '    exit 1',
    '  }',
    '}',
...
export function renderShortcutInstaller(opts: {
  ...
  desktopPath: string
  /** Working directory of the shortcut (defaults to launcher script directory). */
  workingDirectory?: string
  /** Legacy alias for workingDirectory. */
  homeDir?: string
  /** Icon the shortcut shows (an .ico/.png path, or a shell-exe icon spec). */
  iconLocation: string
}): string {
  const { launcherPath, desktopPath, workingDirectory, homeDir, iconLocation } = opts
  const targetWorkingDir = workingDirectory ?? homeDir ?? ''
  return [
