...
/** File name of the VBS wrapper that hides the PowerShell console flash. */
...
/**
 * Render a VBS wrapper that launches the PowerShell launcher without
 * flashing a console window. Used as the shortcut target instead of
 * pointing the LNK directly at powershell.exe with suspicious flags
 * that trigger Windows Defender heuristic detection.
 * @param psLauncherPath - absolute path of launcher.ps1.
 * @returns the VBS wrapper body.
 */
export function renderVbsWrapper(psLauncherPath: string): string {
  const quoted = `"${psLauncherPath.replaceAll('"', '""')}"`
  return [
    "' DeepSeek Harness launcher wrapper (generated by dsh-desktop-launcher)",
    "' Hides the PowerShell console window to avoid a flash on startup.",
    'Dim shell, psLauncher',
    'psLauncher = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\\launcher.ps1"',
    'Set shell = CreateObject("WScript.Shell")',
    `shell.Run "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ` + quoted + `", 0, False`,
    '',
  ].join('\n')
}
 ...
 * the Desktop .lnk pointing at the VBS wrapper, run hidden by the host.
 * The LNK points to wscript.exe + a .vbs file (no -ExecutionPolicy Bypass
 * or -WindowStyle Hidden visible in the shortcut arguments), so Windows
 * Defender heuristic does not flag it as HEUR:Trojan/LNK.Agent.
 * @param opts - the vbs, icon, and working-directory paths.
 * @returns the installer body.
 */
export function renderShortcutInstaller(opts: {
  /** Absolute path of the VBS wrapper. */
  vbsPath: string
