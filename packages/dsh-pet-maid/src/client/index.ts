/**
 * dsh-pet-maid browser half — mounts the whale-maid as a global floating
 * surface and drives it from the host's same-origin `/api/pet-maid/*` JSON
 * endpoints: poll the host snapshot (~800 ms), forward interactions, persist
 * drag positions. The pet is host-global (no session dimension), so it mounts
 ...
 * a dock-mounted pet would vanish there. When the pet is hidden the entry
 * becomes a fixed-position summon button.
 * @module @linxin666/dsh-pet-maid/client
 */
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), 'pet-maid: dictionaries')
  ...
  // Plugin configuration card: one staged form over the `pet-maid` settings
    ...
    if (enabled() && disposeUi === undefined) {
      ...
      const pollNow = (): void => {
        petApi.state().then((snapshot) => {
          ...
          setState('error', 'pet-maid.state transport error')
        })
      }
      ...
      // mounted there would vanish on the new-conversation screen. The entry
      // therefore mounts straight onto document.body via a single React root
      // for the page lifetime: MaidPet portals itself to body when visible,
      // and the hidden-state summon button is fixed-positioned.
      const container = document.createElement('div')
      container.dataset.dshPetMaidRoot = ''
