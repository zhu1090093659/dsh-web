/**
 * Maid-pet spritesheet geometry and animation tracks.
 *
 ...
 *
 * The default atlas is the bundled whale fallback; when the local Codex Pet
 * maid theme is installed the host serves its atlas instead — same 9-row
 * contract, so the track table below works for both. Clawd-style poses alias
 * existing rows: thinking renders the review pose, sleeping holds a static
 * idle frame, attention loops the jumping track.
 *
 * Frame counts and per-frame durations are per-track definitions below; the
 * client prefers the manifest's `frames` field and falls back to an atlas
 * scan (detectFrameCounts) when it is absent. Tracks that do not loop hand
 * off to `fallback`.
 * @module @linxin666/dsh-pet-maid/client/spritesheet
 */
...
/** Rows in the atlas (Codex contract). */
...
/**
 * Track definitions. Durations are tuned for a soft, slow-healing feel;
 * calibrate frame counts against the served manifest (rows may carry 4–8
 * frames — the bundled whale atlas and the maid theme both do).
 */
export const TRACKS: Record<PetAnimation, TrackDef> = {
  ...
  // Clawd-style poses (alias atlas rows 8 / 0 / 4).
  thinking: { frames: [0, 1, 2, 3, 4, 5], durations: [600, 600, 600, 600, 600, 600], loop: true },
  sleeping: { frames: [0], durations: [60_000], loop: true },
  attention: { frames: [0, 1, 2, 3, 4], durations: [300, 300, 300, 350, 350], loop: true },
}
