/**
 * dsh-pet-maid host half — mounts the pet service and its HTTP routes. The
 * browser half (the `./client` entry) renders the whale-maid companion and
 ...
 * link:<dsh-web-ui>/packages/dsh-pet-maid`; the cordis.patch.yml inserts this
 * plugin row.
 * @module @linxin666/dsh-pet-maid
 */
...
import { resolvePetAssetDir } from './asset-source.ts'
import {
  DEFAULT_PET_NAME,
  DISPLAY_INSET_MAX,
  DISPLAY_SIZE_MAX,
...
export {
  ...
  MAID_THEME_ID,
  resolvePetAssetDir,
} from './asset-source.ts'
export type { PetAssetSource } from './asset-source.ts'
...
export const PET_SETTINGS_SCHEMA = z.object({
  ...
  right: z.number().step(1).min(0).max(DISPLAY_INSET_MAX).default(48),
...
export function apply(ctx: Context, config: PetConfig = {}): void {
  ...
  const assetSource = resolvePetAssetDir(config, packageRoot)
  const service = new PetService(ctx, config, assetSource)

  // The settings surface edits the display config through the `pet-maid`
  // namespace. The composition `base` starts as the persisted pet-maid.json
  ...
  const base: PetSettingsSection = {
    ...
    eyeTracking: service.display().eyeTracking,
    miniMode: service.display().miniMode,
  ...
  const routes = makePetRoutes({ service, assetDir: assetSource.dir })
