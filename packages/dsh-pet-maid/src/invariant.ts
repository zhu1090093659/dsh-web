 ...
 * @module @linxin666/dsh-pet-maid/invariant
 */
...
export function runPetInvariants(): void {
  ...
  const phases: readonly ActivityPhase[] = ['idle', 'waiting', 'thinking', 'tool', 'done', 'failed']
  const contract: readonly string[] = [
    'idle', 'running', 'running-right', 'waiting', 'jumping', 'thinking', 'failed',
  ]
  for (const phase of phases) {
    invariant(
      contract.includes(animationForPhase(phase)),
