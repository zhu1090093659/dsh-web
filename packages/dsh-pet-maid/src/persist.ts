/**
 * Maid-pet persistence — tiny JSON store for affinity + display config,
 * written under $DSH_HOME (defaults to ~/.dsh) as `pet-maid.json`.
 * Deliberately minimal: one file, atomic rename write, tolerant read
 * (corrupt file → defaults).
 * @module @linxin666/dsh-pet-maid/persist
 */
...
export interface PetDisplayConfig {
  ...
  eyeTracking: boolean
  /** Edge-dock mini mode: dragging to the right edge tucks the pet in. */
  miniMode: boolean
}
...
export const DEFAULT_PET_NAME = '女仆鲸鱼娘'
...
export function loadPetPersist(dir: string = petHomeDir()): PetPersist {
  try {
    ...
    const display: PetDisplayConfig = {
      ...
      eyeTracking: typeof rawDisplay.eyeTracking === 'boolean' ? rawDisplay.eyeTracking : base.display.eyeTracking,
      miniMode: typeof rawDisplay.miniMode === 'boolean' ? rawDisplay.miniMode : base.display.miniMode,
    }
    return {
