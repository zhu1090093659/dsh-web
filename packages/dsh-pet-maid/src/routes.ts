/**
 * Maid-pet HTTP routes — the browser half talks to the host through plain
 * same-origin JSON endpoints (`/api/pet-maid/*`) and loads the pet atlas
 ...
 * The atlas directory is resolved at apply time (local Codex Pet theme or
 * the bundled fallback), see asset-source.ts.
 * @module @linxin666/dsh-pet-maid/routes
 */
...
/** Relative (to the resolved atlas dir) asset files exposed under the prefix. */
const ASSET_FILES = [
...
/** Build the full route family (API + assets) for one service + atlas dir. */
export function makePetRoutes(deps: { service: PetService; assetDir: string }): WebRoute[] {
  const { service, assetDir } = deps
  const apiRoutes: WebRoute[] = [
    ...
    postRoute(`${PET_API_PREFIX}/set-config`, (body) => service.setConfig({
      ...
      ...(typeof body.eyeTracking === 'boolean' ? { eyeTracking: body.eyeTracking } : {}),
      ...(typeof body.miniMode === 'boolean' ? { miniMode: body.miniMode } : {}),
    })),
  ...
  const assetRoutes: WebRoute[] = ASSET_FILES.map((file): WebRoute => ({
    ...
    handler: (req: IncomingMessage, res: ServerResponse): Promise<void> | void => {
      ...
      return readFile(join(assetDir, file.name)).then((body) => {
        res.writeHead(200, {
