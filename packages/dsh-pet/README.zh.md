...
| 工作指示器 | 原生 `Deep diving...` 回合状态在模型工作时显示一只喷水鲸鱼；装饰对读屏隐藏，减少动态效果偏好下保持静态 |
...
- **挂载点**：`document.body`（全局 React 根，始终显示：无会话 / 新会话 / 会话中都可见——旧挂载点 `conversation.composer.dock` 只在活动会话里渲染，新会话里宠物消失）；组件内部用 `createPortal` 渲染全局浮层。客户端还会观察原生语义回合状态行，在文字前添加唯一的工作鲸鱼装饰，并随插件 effect 一并清理。
