...
| 网页宠物选择 | 插件设置卡片列出宿主网页注册表中的全部宠物；切换即持久化网页 `petId`，网页精灵立即更换 |
| 网页宠物独立命名 | 在网页悬浮面板改名；宿主按网页注册表 pet id 保存每只网页宠物的名字（旧版平铺名字自动迁移） |
...
| 共享伴侣数据 | 网页与桌面互动都调用同一个宿主持有的 `PetService`，亲密度、小鱼干库存、互动冷却、会话与碎碎念气泡、轮次奖励只有一个事实来源；网页 `petId`/`names` 与 Electron 本地 `modelId`/`modelAliases` 彼此独立 |
| 受管桌面生命周期 | 桌面宠物默认关闭；启用后随带 WebServer 的 DSH Host 启停，从托盘退出还会把持久化开关同步写回关闭；连续启动失败或崩溃会按退避策略自动重试最多 3 次，耗尽后将持久化开关写回关闭 |
...
聚合安装和单独安装共用 `pet` 设置命名空间，但网页与桌面不共用模型身份。网页设置卡编辑宿主持有的网页选择与布局，以及桌面生命周期/窗口控制；网页改名保存在宿主的 `$DSH_HOME/pet.json`。桌面设置抽屉通过鉴权宿主桥回写窗口控制，而所选 `modelId` 与模型列表中的 `modelAliases` 仍是 Electron 本地偏好。

| 字段 | 默认值 | 含义 |
|---|---:|---|
| `enabled` | `true` | 宠物活动与表现路由的总开关 |
| `petId` | 注册表默认值 | 网页表现从宿主网页注册表中选中的宠物；已移除的 id 会回退到注册表默认值，且不会改变桌面模型 |
...
|   |-- service.ts           # PetService：网页宠物选择 + 共享状态/经济 + 配置
...
|   |-- treats.ts            # 小鱼干库存账本
|   |-- persist.ts           # 持久化（$DSH_HOME/pet.json：网页选择/名字 + 共享经济 + 网页显示）
...
官方会话事件（turn/step/chunk/tool）----\
                                                    > PetService（宿主）<-- 网页注册表（内置 + 自定义宠物）
可选兼容 activity/status ------------------/
                                          |                         |
                               /api/pet/* JSON            鉴权回环 SSE（共享伴侣状态）
                                          |                         |
                             pet-client（浏览器）             Electron 桌面宠物
                                  |                                  ^
                    PetSprite 浮层（portal + rAF）                    |
                                                       Electron 模型目录 + 本地 modelId/别名
```
...
- **选择与命名**：网页 `petId` 存于设置命名空间；网页每只宠物的名字存于宿主 `$DSH_HOME/pet.json` 的 `names`，通过网页悬浮面板对当前网页注册表宠物改名。旧版安装的平铺 `name` 自动迁移到鲸鱼娘名下。这些字段不会选择或重命名桌面模型：Electron 在本地桌面偏好中独立保存自己的 `modelId` 与模型列表 `modelAliases`。
...
- **通信**：浏览器 ↔ 宿主走同源 `/api/pet/*` JSON 端点，每只宠物的图集从 `/pet/<id>/<spritesheetPath>` 加载。单独安装时，主设置 scope 不可用才回退到仅限回环地址的 `/api/pet/settings`；`/api/pet/runtime` 向设置弹窗提供状态、显式安装与取消。受管子进程通过令牌鉴权的 `/api/pet/native/*` 状态、互动与 SSE 路由共享亲密度、小鱼干、冷却、会话气泡和轮次奖励；它不直接读写宿主 `$DSH_HOME/pet.json`，模型选择与别名保存在 Electron 本地偏好中。
...
- Electron 进程消费宿主 API，并在本地保存原生窗口偏好、自己的模型选择与模型列表别名。亲密度、小鱼干库存、互动冷却、会话与碎碎念气泡、轮次奖励仍以 `PetService` 为唯一事实来源；网页 `petId` 与每只宠物的 `names` 则是独立的宿主网页注册表状态。
