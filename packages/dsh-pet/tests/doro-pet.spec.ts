...
describe('doro pet manifest', () => {
  ...
  it('declares every track with the 42ms default duration', () => {
  ...
  it('marks the one-shot acts and the drag-only loop', () => {
    if (!res.ok) throw new Error('manifest rejected')
  ...
  it('work rule: 10s tick, 50% success, result window covering both animations', () => {
    if (!res.ok) throw new Error('manifest rejected')
  ...
  it('sleep restores energy and the wash mode restores mood every second', () => {
    if (!res.ok) throw new Error('manifest rejected')
  ...
  it('roam rolls every 15s at 30% across the four directions', () => {
    if (!res.ok) throw new Error('manifest rejected')
  ...
  it('idle director plays three acts, each at 30%', () => {
    if (!res.ok) throw new Error('manifest rejected')
  ...
  it('plays the struggle loop while the chrome reports a drag', () => {
    if (!res.ok) throw new Error('manifest rejected')
    expect(res.manifest.gameplay?.dragState).toBe('struggle')
    expect(res.manifest.frames2d!.tracks['struggle']).toBeDefined()
  })
