...
`session.create` (workspace id only — the phone never names a working
 ...
 The canonical `/m/` page owns a same-scope manifest and Service Worker; its
 cache is limited to the static shell and an offline page, never mobile API
 responses, session data, or commands.
