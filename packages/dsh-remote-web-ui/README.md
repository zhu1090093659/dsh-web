...
- **A paired device is a full-control credential.** With host mode active it reaches the complete host API — chat, sessions, settings, credentials, agent presets, deliverables, and standalone plugin RPC channels such as dsh-mnemon's `/dsh-mnemon-*` paths — mirroring the SDK's own stance that a loopback desktop is trusted. Standalone plugin RPC calls are rewritten through `/remote` rather than requiring `--trusted-host`. Only the three control planes (pairing, self-update, plugin install/remove) stay physically local. Only pair devices you control; 停止 or per-device 取消配对 revokes immediately.
