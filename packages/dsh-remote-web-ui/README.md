...
- **Security**: one active token at a time (a refresh invalidates the old link; the link stays re-usable within its expiry window so a scan handed between browsers can complete pairing; it expires). 停止 revokes every paired device and the current token — the `/remote` channel cuts them off on their next request. Pairing is this plugin's access control for the `/remote` channel; direct `/api` on a LAN-exposed bind is governed by the harness fence + browser auth (see Security model). A page that is the local machine keeps using `/api` directly: loopback addresses (127.0.0.1) and the official Desktop application's own page (`dsh-app://app/`, whose Electron shell forwards same-origin calls to its own authenticated host) are not remote origins, so they are never asked to pair. A paired device is a **full-control credential** (see Security model).
...
- **The official Desktop application is recognized by its page scheme**: the local-machine test accepts loopback addresses and the Desktop shell's own `dsh-app:` page scheme. A Desktop release that changes that scheme or ships a different shell degrades to an ordinary remote origin that must pair; the single source of truth is `DESKTOP_SHELL_SCHEME`, shared by the host and browser halves through one rule table.
