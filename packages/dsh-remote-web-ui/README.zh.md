...
- **PC 侧（远程桌面配对）**：同一份二维码链接也能配对 PC 浏览器。从面板复制链接，在另一台电脑的浏览器打开（局域网 URL 或隧道 URL 均可）；接受往返后，完整 Web GUI 在那台设备上运行——工作区、会话、聊天、切模型——流量走门控的 `/remote/api` 通道而非 `/api`。手机进 `/m/`，PC 进桌面 UI：一枚令牌、一套配对流、两种界面。未配对的 PC 只看到「此设备未配对」横幅，没有任何工作区数据。
- **安全**：一个有效的一次性令牌（刷新会使旧链接失效；已接受的令牌不可复用；令牌会过期）。停止会撤销每一台已配对设备与当前令牌——已配对设备在下一次请求时被切断。远程数据通道全部是本插件自己的路由，逐请求受已配对设备 cookie 门控：手机走 `/m/api`，非 loopback origin 打开的桌面浏览器走 `/remote/api`（连接插件的 `/api` 围栏会拒绝这类 origin，本插件也绝不要求你信任公网 host——**切勿为隧道域名传 `--trusted-host`**，那会把无门控的 `/api` 对整个互联网敞开；下方姿态探测会让这种回退立刻可见）。loopback（127.0.0.1）继续直接使用 `/api`。
...
- **姿态探测**：插件用伪造 Host 头（公网 base 与每个 LAN base）探测自身 `/api` 围栏。任何非 403 结果——重新加回的 `--trusted-host`，或 `--host 0.0.0.0` 下 SDK 的局域网自动信任——都会以 CRITICAL 日志与配对面板红色横幅呈现，因为 SDK 围栏是插件唯一无法上门的接缝。
...
二维码在隧道报告其 URL 前保持仅局域网，且隧道重启会铸一枚**新的** hostname——插件清除旧链接并铸一枚新的，用户永远不必触碰配置。注意 quick tunnel 是公网的：任何拿到 URL 的人都能加载静态页；已配对设备 cookie 门才是真正的围栏（手机的 `/m/api` 带方法白名单、远程桌面的 `/remote/api`），连接插件的 `/api` 围栏则直接拒绝隧道化主机——被隧道化的 Host 永远不进入连接信任围栏，因此 **auto tunnel 工作无需任何 profile 或 harness 定制**（也绝不可加：切勿为隧道域名传 `--trusted-host`）。

### 手动隧道（自带）

二维码链接通常是局域网 URL，所以家外的手机无法使用。把隧道指向 dsh web 端口，并告知插件其公网地址——二维码随后由隧道 URL 构建。涉及一个钮，一个被明确禁止的 flag：
...
- **切勿传 `--trusted-host <隧道域名>`**（dsh web flag）：它让 SDK 的 `/api` 围栏接受公网主机，等于把无门控的完整 host API 对全互联网敞开——配对不门控 `/api`（任何插件都无法做到；围栏是 SDK 自己的接缝）。若该 flag 仍被加上，姿态探测会发现敞开的围栏并在配对面板显示红色横幅；移除 flag 即闭合。
...
# 2. 照常启动 dsh web——不要 --trusted-host（它会把 /api 敞开给互联网；
...
- Quick tunnel 是公网的：任何拿到 URL 的人都能加载静态页。已配对设备 cookie 门才是真正的围栏——未配对设备每个 `/m/api` 与 `/remote/api` 调用都 403——所以请保持 `requirePairingForLan` 开启，且绝不为隧道主机加 `--trusted-host`。
...
- **`api/gate` 瀑布**（packages/client/connection）：/api 路由与事件 WebSocket 升级被*设计*为在信任围栏后发出该事件，插件可据此实施应用层访问控制。当前已发布的 SDK 线**并不**发出它（监听器为将来具备该 seam 的部署保持挂载）——这正是远程桌面通道 `/remote/api` 与姿态探测存在的原因：配对执行力落在本插件自己的路由上，而（插件无法触达的）`/api` 围栏状态是被验证的，不是被假设的。
...
公网路径是经隧道的同一往返（见「通过互联网远程访问」）：loopback mint → 手机打开公网二维码 URL → accept → 完整 UI。只有 `publicBaseUrl`（插件配置）命名隧道主机——**绝不** `--trusted-host`；桌面面板仍在 `http://127.0.0.1` 打开。
