...
- **配对设备是完全控制凭据。** host 模式下它可达完整 host API——聊天、会话、设置、凭据、Agent 预设、产出物，以及 dsh-mnemon `/dsh-mnemon-*` 等独立插件 RPC 通道——与 SDK 对回环桌面的信任一致。独立插件 RPC 调用会经 `/remote` 重写，无需配置 `--trusted-host`。只有三个控制面（配对、自更新、插件安装/卸载）保持物理本地。只配对你控制的设备；停止或逐设备取消配对立即撤销。
