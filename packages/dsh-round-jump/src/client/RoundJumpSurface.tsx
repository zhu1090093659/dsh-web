...
export function RoundJumpSurface(props: RoundJumpProps): ReactElement {
  ...
  const pointerInside = (x: number, y: number): boolean => {
    ...
    return el !== null && el.closest('[data-round-jump-popup]') !== null
  }
  ...
  return createPortal(
    <div data-round-jump-popup="">
      <style>{`
        ...
        .rj-panel { pointer-events: auto; flex: none; width: 280px; max-height: 70vh; margin-right: 8px; display: flex; flex-direction: column; background: #fff; border: 1px solid #d0d7de; border-radius: 10px; box-shadow: 0 8px 28px rgba(0,0,0,.18); overflow: hidden; font: 13px/1.45 system-ui, -apple-system, "Segoe UI", sans-serif; color: #1f2328; transform: translateX(calc(100% + 16px)); opacity: 0; transition: transform 160ms ease, opacity 160ms ease; }
        ...
        .rj-list { overflow-y: auto; padding: 4px; display: flex; flex-direction: column; gap: 2px; }
        .rj-item { display: flex; gap: 8px; align-items: baseline; padding: 6px 8px; border-radius: 6px; cursor: pointer; border: none; background: transparent; text-align: left; color: inherit; font: inherit; }
        ...
        .rj-load-all { width: 100%; padding: 6px 10px; border: 1px solid #d0d7de; border-radius: 6px; background: transparent; color: #4f8ff7; font: 600 12px system-ui, sans-serif; cursor: pointer; }
      ...
      <div className={`rj-wrap${open ? ' rj-open' : ''}`}>
        <div className="rj-panel" role="dialog" aria-label="对话轮次跳转">
