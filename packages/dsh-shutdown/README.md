...
A single power button (Windows shutdown icon) at the bottom of the DSH Web sidebar, right beside the settings trigger. Clicking it opens a confirm dialog; confirming asks the host process to exit DeepSeek Harness gracefully.

## What it does

- **One button only**: the sidebar footer gets exactly one extra control — a power-icon trigger matching the settings button geometry (36px circle in the rail, inline icon in the wide row).
- **Confirm dialog**: clicking the button opens a modal asking for confirmation, because exiting ends the dsh web process and may interrupt running sessions and tasks. The dialog can be bypassed through the `confirmShutdown` setting.
...
Restart `dsh web`; the power button appears at the bottom of the sidebar next to the settings trigger.
...
- The button lives in the Web GUI sidebar footer; there is no TUI equivalent.
