...
DSH Web 页面右下角的一个关机按钮（Windows 关机样式图标，浮动显示）。点击按钮弹出确认框；确认后请求宿主进程优雅退出 DeepSeek Harness。

## 功能

- **只有一个按钮**：页面右下角一个浮动关机按钮——46px 圆形图标按钮，带阴影，始终可见，不依赖侧边栏布局。
- **确认弹窗**：点击按钮先弹确认框，因为退出会结束 dsh web 进程，可能中断正在运行的会话与任务；可用 `confirmShutdown` 设置关闭确认。确认后宿主应答，页面会先自行关闭（`window.close()`；浏览器禁止脚本关闭标签页时回退为空白页），随后进程退出。
