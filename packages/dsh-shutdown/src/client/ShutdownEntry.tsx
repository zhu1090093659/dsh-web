/**
 * The sidebar footer power trigger: a Windows-style shutdown icon button
 * beside the settings trigger. Clicking it opens a confirm dialog; the
 * confirmed request POSTs to the loopback-only /api/dsh-shutdown route, and
 * the host process exits gracefully (ctx.appExit) a beat after the response.
 */
import { useCallback, useEffect, useState } from 'react'
import { createPortal } from 'react-dom'
import type { InjectFace, PropsLocale } from '@deepseek-ai/dsh-client-ui-slots'
import { requestShutdown } from './shutdown-api.ts'
...
/** Entry props: the footer seat's column state + the locale seat + the face. */
...
export function ShutdownEntry(props: ShutdownEntryProps) {
  ...
  const performShutdown = useCallback(async () => {
    ...
    try {
      ...
      // The host acknowledges first and exits a beat later; the page dies
      // with the process, so there is nothing to render after this point.
    } catch (caught) {
