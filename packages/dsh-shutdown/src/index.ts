...
export const SHUTDOWN_GUIDANCE = '本机已安装 dsh-shutdown 插件（DSH Web GUI 的一键关机）：界面右下角的关机样式浮动按钮，点击弹出确认框，确认后请求宿主进程优雅退出（经 ctx.appExit，先回收插件树再退出；无 appExit 时回退 process.exit(0)）。限制：退出会终止 dsh web 进程，正在运行的会话/任务可能中断；路由仅限 loopback。用户提到「关机 / 退出 DSH / 关闭 DeepSeek Harness」时即指本插件，请据此协作。'
...
/** Services required before the shutdown surfaces can mount. */
