...
| --- | --- |
| Host management | CRUD, search, connection test; collapsible grouping by environment / tags with per-group batch test; config stored in `$DSH_HOME/dsh-ssh.json`; supports key / password / ssh-agent auth (OpenSSH agent / Pageant), passphrase keys, ProxyJump jump hosts (multi-level) |
...
- Passwords / key passphrases are stored in plain text in `$DSH_HOME/dsh-ssh.json`, file mode 0600, directory 0700 (the same trust model as ssh-skill writing passwords into ssh-config comments).
...
- Host config: `$DSH_HOME/dsh-ssh.json` (versioned JSON, atomic write)
