...
- macOS 只允许 `/usr/bin/caffeinate -i -w <pid>`；Windows 只允许从 `SystemRoot` 解析的 Windows PowerShell 固定 helper 和 `ES_CONTINUOUS | ES_SYSTEM_REQUIRED`。不得请求亮屏、修改电源计划、依赖 PATH 或要求管理员权限。
- helper 必须 `shell: false`、失败有界退避，并在设置关闭、插件卸载和 Host 退出时清理。Linux/其他平台只报 `unsupported`。
...
- 变更账本、协议、runner、cron 或 power 时补对应单测；原生 helper 只在 `DSH_POWER_SMOKE=1` 且平台为 Windows/macOS 时运行 smoke。
