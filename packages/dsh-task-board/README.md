...
- **Task board UI**: a sidebar entry below New Session shows icon and text in the wide sidebar and a centered icon in the collapsed rail, including standalone installation on the official default skin; the board provides five kanban columns, search, task details, archive/restore, execution history, and links to execution transcripts. Archived tasks are read-only except for restore, delete, and transcript viewing, and cannot run manually or on schedule until restored.
