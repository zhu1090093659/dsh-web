...
| Live config card | Settings → Plugin config → Web Plugins → "Image understanding" card edits `baseURL` / `apiStyle` / `model` / API key / default instruction / bounds. The card writes this profile entry's own configuration — the plugin's `Config` schema IS its settings page, so the fields the card edits and the fields the runtime reads are the same ones; the host restarts the entry with the new values, so a save is effective immediately without a host restart |
...
| Native image toggle | rc.8: the settings card's "Native image requests" section reports the current default model's image-input state and toggles the DeepSeek adapter catalog entry (`inputModalities` in the `llm-deepseek` profile entry's configuration) through the loopback route pair `GET` / `POST /describe-image/native-images`. Enabled: sent images reach the model natively and `describe_image` hides from that model's toolset; disabled: the legacy rewrite applies. Hosts without the adapter entry render the section with an unsupported hint |
...
- The native-image toggle routes are loopback-only with the same same-origin fence; they write
  only the official `llm-deepseek` entry's configuration through the host settings seam
  (revision-fenced, validated by the adapter schema) and never touch credentials.
...
start immediately, no host restart needed. (Difference from upstream: upstream validates eagerly at load;
...
image-input verdict and a toggle that rewrites the `llm-deepseek` entry's configuration through the
