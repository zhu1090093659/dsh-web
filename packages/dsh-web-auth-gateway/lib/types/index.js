...
function saveCredential(username, password) {
    ...
    const value = { username, salt, passwordHash: scryptSync(password, salt, 32).toString('hex') };
...
export function apply(ctx, config = {}) {
            ...
            if (path === '/_dsh_auth/login') {
                ...
                if (req.method === 'POST') {
                    ...
                    if (credential === undefined && username.length > 0 && password.length >= 8) {
                        ...
                        ok = verifyPassword(credential, username, password);
                    if (ok) {
        ...
        server.on('upgrade', (req, socket, head) => {
            ...
            const upstream = connect(ctx.webServer.port, ctx.webServer.host, () => {
                ...
                for (const [key, item] of Object.entries(headers)) {
                    if (item !== undefined)
                        raw += `${key}: ${Array.isArray(item) ? item.join(', ') : item}\r\n`;
                }
                upstream.write(`${raw}\r\n`);
                if (head.length > 0)
                    upstream.write(head);
    ...
    const configRoute = {
        ...
        handler: async (req, res) => {
            if (req.method === 'GET') {
                json(res, 200, { ok: true, value: source(), writable: settingsScope !== undefined });
                return;
            }
            if (req.method !== 'POST') {
            ...
            if (settingsScope === undefined) {
                json(res, 503, { ok: false, error: 'settings-unavailable' });
                return;
            }
            try {
                ...
                if ('sessionTtlHours' in input) {
                    if (!Number.isInteger(input.sessionTtlHours) || input.sessionTtlHours < 1 || input.sessionTtlHours > 720)
...
function proxyHttp(req, res, host, port) {
    ...
    proxy.on('error', () => { if (!res.headersSent)
