...
export const zh = {
  ...
  'mode.annotate': '批注',
  'action.capture': '匿名截图',
  'action.interactive': '交互浏览',
  'action.captureInteractive': '截取交互窗口',
  'action.send': '发送给模型',
  'action.clear': '清空',
  'action.undo': '撤销',
  'action.upload': '上传图片',
  ...
  'status.openingBrowser': '正在打开交互浏览器…',
  'status.browserOpen': '交互浏览器已打开；完成登录和跳转后点「截取交互窗口」',
  'status.capturingInteractive': '正在截取交互窗口当前页面…',
  'status.sending': '发送中…',
  'status.inserted': '已插入对话草稿，补充说明后发送即可',
  'status.idle': '截图后在图上绘制批注，再发送给模型 OCR 识别',
  'error.noUrl': '请先在地址栏输入网址',
  'error.capture': '截图失败：{message}',
  'error.browser': '交互浏览器失败：{message}',
  ...
  'region.placeholder': '说明这个圈选区域的问题',
  'region.label': '区域 {number}',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  'action.capture': 'Anonymous screenshot',
  'action.interactive': 'Interactive browser',
  'action.captureInteractive': 'Capture interactive window',
  ...
  'status.openingBrowser': 'Opening the interactive browser…',
  'status.browserOpen': 'Interactive browser opened; sign in and navigate, then capture its current page',
  'status.capturingInteractive': 'Capturing the current interactive page…',
  ...
  'error.browser': 'Interactive browser failed: {message}',
  ...
  'region.placeholder': 'Describe the issue in this selected region',
  'region.label': 'Region {number}',
