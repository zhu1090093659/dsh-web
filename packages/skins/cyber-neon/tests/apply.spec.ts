...
/**
 * cyber skin apply/dispose contract: the body attribute the stylesheet is
...
afterEach(async () => {
  ...
  delete document.body.dataset.dshCyberNeon
})

describe('cyber skin apply', () => {
  it('mounts the scene: body attribute, scene layer, background override', async () => {
    fiber = await mount()

    expect(document.body.dataset.dshCyberNeon).toBe('')
  ...
  it('retracts everything on fiber dispose', async () => {
    ...
    expect(document.body.dataset.dshCyberNeon).toBeUndefined()
