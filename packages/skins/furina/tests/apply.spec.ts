...
/**
 * Furina skin apply spec — the template contract: the body
...
describe('Furina skin apply', () => {
  it('sets the body attribute and retracts it on dispose', async () => {
    fiber = await mount()
    expect(document.body.hasAttribute('data-dsh-furina')).toBe(true)
    await fiber.dispose()
    expect(document.body.hasAttribute('data-dsh-furina')).toBe(false)
  })
