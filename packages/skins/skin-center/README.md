...
- List: shows "官方默认" (official default) plus every valid, built skin discovered from the resolved skins directory, including custom skins under `$DSH_SKINS_DIR`, with its name, tagline, and accent color; the currently active target carries the Active marker.
...
skins/skin-center/
  ...
  src/client/skin-roster.ts                          # dynamic host roster loader + generated compatibility fallback
...
- Skin enumeration: host route `/api/skin-center/skins` scans the resolved skins directory on demand, validates each `skin.json` plus `lib/client.js`, and returns browser-safe metadata sorted by `order` and id; this includes valid custom skins under `$DSH_SKINS_DIR`. `generated/skins.ts`, produced by `scripts/skin-center-bundles`, is the compatibility fallback for an older host, a failed request, or an invalid response. **It contains metadata only, no embedded bundle text**: cold start does not parse the ~700KB base64 art assets, and the generated file is reproducible across machines (no build-machine absolute paths).
...
- Active-skin detection: `window.__DSH_BOOT__.entries` only contains enabled entries, compared against the dynamically loaded roster packages; no match means official default.
...
- [x] The list contains the official default plus all valid built skins, including `$DSH_SKINS_DIR` custom skins; the currently active one is marked
