...
export default function defineSkinHooks() {
  return {
    apply(ctx) {
      ...
      // ----------------------------------------------------- title block
      // The corner stamp of a drawing sheet. The date is generated once per
      // activation, so a skin switch refreshes it.
      const titleblock = doc.createElement('div')
      titleblock.className = 'dsh-bp-titleblock'
      titleblock.setAttribute('aria-hidden', 'true')
      const tbHead = doc.createElement('div')
      tbHead.className = 'dsh-bp-tb-head'
      tbHead.textContent = 'DSH · 本地构建'
      const tbRows = doc.createElement('div')
      tbRows.className = 'dsh-bp-tb-rows'
      const today = new Date()
      const pad2 = (n) => String(n).padStart(2, '0')
      const stampDate = `${today.getFullYear()}-${pad2(today.getMonth() + 1)}-${pad2(today.getDate())}`
      for (const [key, value] of [['SHEET', SHEET], ['SCALE', SCALE], ['REV', '1.0.0'], ['DATE', stampDate]]) {
        tbRows.append(span('dsh-bp-tb-key', key), span('dsh-bp-tb-val', value))
      }
      titleblock.append(tbHead, tbRows)
      doc.body.append(titleblock)
